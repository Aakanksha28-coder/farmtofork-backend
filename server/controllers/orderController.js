...
const { notifyUser } = require('../utils/orderNotify');
const {
  orderPlacedCustomer,
  newOrderFarmer,
  ...
  try {
    ...
    try {
      ...
      await notifyUser({
        ...
        template: orderPlacedCustomer(order, customer.name, farmer?.name)
      });
      ...
      if (farmer?.email) {
        await notifyUser({
          ...
          email: farmer.email,
          template: newOrderFarmer(order, customer.name, farmer.name)
        });
      }
    } catch (err) {
    ...
    try {
      ...
      if (customer?.email) {
        await notifyUser({
          ...
          email: customer.email,
          template: statusUpdateCustomer(order, status, note, customer.name)
        });
      }

      if (farmer?.email && (status === 'delivered' || status === 'received')) {
        await notifyUser({
          userId: farmer._id,
          orderId: order._id,
          title: `Order #${shortId}: ${label}`,
          ...
          email: farmer.email,
          template: statusUpdateFarmer(order, status, farmer.name)
        });
      }
    } catch (err) {
